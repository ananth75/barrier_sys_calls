...
static bool barrier_dequeue_started = 0;
...
asmlinkage long sys_barrier_wait(unsigned int barrier_id)
{
    ...
    list_for_each_entry(ibarrier_iter, &barrier_global_list, m_barrier )
    {
        ...
        if(ibarrier_iter->m_barrier_id == barrier_id && ibarrier_iter->m_barrier_pid == task_tgid_vnr(current))
        {
            printk("\n sys_barrier_wait from process id %d found with barrier id %d \n", ibarrier_iter->m_barrier_pid, ibarrier_iter->m_barrier_id);
            is_barrier_found = 1;
            break;
        }
    }
    ...
    {
        printk("\n sys_barrier_wait call barrier not found ");
    ...
    if(ibarrier_iter->m_thread_current_count < ibarrier_iter->m_thread_count -1)
    {
    ...
    {
        printk("Las thread in the barrier \n");
...
asmlinkage long sys_barrier_init(unsigned int count, int *barrier_id, signed int timeout)
{
    p_eosi_barrier_t ibarrier, ibarrier_iter;
    int t_count;
    
    if ((ibarrier = kmalloc(sizeof(eosi_barrier_t), GFP_KERNEL)) == NULL)
    {
        printk("Kmalloc failed for barrier\n");
    }
    
    t_count = 1;
    memset(ibarrier, 0, sizeof( eosi_barrier_t));
...
asmlinkage long sys_barrier_destroy(unsigned int barrier_id)
{
    ...
    list_for_each_safe(next, temp, &barrier_global_list)
    {
        ...
        if(ibarrier->m_barrier_pid == task_tgid_vnr(current) && ibarrier->m_barrier_id == barrier_id)
        {
            ...
            printk("\nBarrier id  %d destroyed with tgid %d", ibarrier->m_barrier_id, ibarrier->m_barrier_pid);
